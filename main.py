...
path_to_save = os.path.join(os.getcwd(), 'result-ascii-gray')
path_to_video = os.path.join(os.getcwd(), 'test_720P_1500K_217812511.mp4')
video = cv2.VideoCapture(path_to_video)

if video.isOpened() and SCALE == -1:
    ret, frame = video.read()
    h, w, _ = frame.shape
    SCALE = WIDTH / w
    NEW_WIDTH = int(w * SCALE)
    NEW_HEIGTH = int(h * SCALE)
...
while video.isOpened():
    ...
    for row in range(NEW_HEIGTH):
        for col in range(NEW_WIDTH):
            gsval = gscale67[int((gray[row, col]*len_gscale67)/255)]
